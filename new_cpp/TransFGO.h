...
public:
    ...
    struct TransFGOConfig
    {
        ...
        TransFGOConfig(double branch_res, double eps, double prox_thresh,
                       const Eigen::Vector3d &space_sz, bool saturated = true)
            : branch_resolution(branch_res), epsilon_t(eps), prox_threshold(prox_thresh),
              space_size(space_sz), use_saturated(saturated) {}
    };
    ...
    TransFGO(double branch_resolution, double epsilon_t, double prox_threshold,
             const Eigen::Vector3d &space_size, bool use_saturated = true)
    ...
    Eigen::Vector3d solve(const std::vector<int> &ids,
    ...
    // Alternative solve method for pre-processed data
    Eigen::Vector3d solveWithPreprocessedData(const std::vector<Eigen::Vector3d> &pert_rot_n_2D,
                                              const std::vector<Eigen::Vector3d> &endpoints_3D,
                                              const std::vector<int> &ids,
                                              const Eigen::Matrix3d &R_opt,
                                              const Eigen::Matrix3d &intrinsic);

    // Get translation candidates only (for advanced users)
    ...
    std::pair<double, Eigen::Vector3d> pruneTCandidates(const Eigen::Matrix3d &R_opt,
                                                        const Eigen::Matrix3d &intrinsic,
                                                        const std::vector<Eigen::Vector3d> &pert_rot_n_2D,
                                                        const std::vector<Eigen::Vector3d> &endpoints_3D,
                                                        const std::vector<int> &ids,
