...
set_name = "datasets_0228";
output_filename= "record/"+dataset_idx+"_rotation_result_record.mat";
% 
% set_name = "datasets_0228_dino";
% output_filename= "record/"+dataset_num+"_rotation_result_record_dino.mat";

datasetname=set_name+"/"+dataset_idx+"/";



load(datasetname+"lines3d.mat");
% kernel function f(k) for saturated interval stabbing f(k)
kernel_1 = @(x) exp(-x+1);
kernel_2 = @(x) x^-2;
kernel_3 = @(x) x^-8;
kernel_4 = @(x) 2^(-x+1)/x;
...
Record_My_sat_clustered     =table('Size', [numRows, length(column_names)],'VariableTypes', columnTypes,'VariableNames', column_names);
Record_RAL_sat_clustered    =table('Size', [numRows, length(column_names)],'VariableTypes', columnTypes,'VariableNames', column_names);
Record_My_sat_unclustered   =table('Size', [numRows, length(column_names)],'VariableTypes', columnTypes,'VariableNames', column_names);
Record_My_unsat_unclustered =table('Size', [numRows, length(column_names)],'VariableTypes', columnTypes,'VariableNames', column_names);
Record_RAL_unsat_unclustered=table('Size', [numRows, length(column_names)],'VariableTypes', columnTypes,'VariableNames', column_names);
%%  experiment
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  params  %%%%%%%%%%%%%%%%%
% rotation bnb
verbose_flag=0;
epsilon_r = 0.0125;
branch_resolution = pi/2048;
sample_resolution = pi/512;
% translation ransac
sampleSize=4;
...
parfor num =0:total_picture 
    ...
    %%%%%%  result record
    Record_My_sat_clustered(num+1,:)={num*10,min_error_sat_My,0,0,time_sat_My,num_candidate_sat_My,...
    ...
    %%%%%%  result record
    Record_My_sat_unclustered(num+1,:)={num*10,min_error_sat_My_unclustered,0,0,time_sat_My_unclustered,num_candidate_sat_My_unclustered,...
    ...
    % %%%%%%%%% result record
    % Record_RAL_sat_clustered(num+1,:)={num*10,min_error_sat_RAL,0,0,time_sat_RAL,num_candidate_sat_RAL,...
    %                         length(data_3d_v_clustered),length(unique(id_cluster(gt_inliers))),...
    ...
    % 
    % %%%%%%%% result record
    % Record_My_unsat_unclustered(num+1,:)={num*10,min_error_plain_My,0,0,time_plain_My,num_candidate_plain_My,...
    ...
    % %%%%%%%%% result record
    % Record_RAL_unsat_unclustered(num+1,:)={num*10,min_error_plain_RAL,0,0,time_plain_RAL,num_candidate_plain_RAL,...
    %                         length(data_3d_v),nnz(lines2d(:,end)>=0),...
...
Record_My_sat_clustered(all(Record_My_sat_clustered{:, :} == 0, 2), :) = [];
% Record_RAL_sat_clustered(all(Record_RAL_sat_clustered{:, :} == 0, 2), :) = [];
Record_My_sat_unclustered(all(Record_My_sat_unclustered{:, :} == 0, 2), :) = [];
% Record_My_unsat_unclustered(all(Record_My_unsat_unclustered{:, :} == 0, 2), :) = [];
% Record_RAL_unsat_unclustered(all(Record_RAL_unsat_unclustered{:, :} == 0, 2), :) = [];
...
%%
% save(output_filename,"Record_RAL_unsat_unclustered","Record_My_unsat_unclustered","Record_My_sat_unclustered","Record_RAL_sat_clustered", ...
%                             "Record_My_sat_clustered","sampleSize","sample_resolution","epsilon_t","epsilon_r","branch_resolution","ransac_iterations");
% % 
