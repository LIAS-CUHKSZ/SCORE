...
sample_reso = pi/128;     % resolution for interval analysis
prox_thres = branch_reso; % for clustering proximate stabbers
epsilon_r = 0.015;

% configure table to record results
total_img=2000;
column_names=...
    ...
    for pred_flag = 0:1
        ...
        for two_or_eight = 0:1
            ...
            parfor num = 1:total_img
                ...
                retrived_closest_pose = readmatrix(data_folder+"retrived_closest_pose/frame_"+frame_id+"_retrived_pose.csv"); % pose of the most similar retrived image
                ...
                for k = 1:num_q
                    ...
                    [R_opt,best_score,num_candidate,time,~,~] = ...
                        Sat_RotFGO(n_2D,v_3D,ids,kernel_buff_SCM_ML_lists{k},branch_reso,epsilon_r,sample_reso,prox_thres,initial_branch);
            ...
            print_statsitics(Record_CM_FGO,Record_SCM_trunc,Record_SCM_ML_lists)
        end
    end
end

%% subfunctions
function []=print_statsitics(Record_CM_FGO,Record_SCM_trunc,Record_SCM_ML_lists)
