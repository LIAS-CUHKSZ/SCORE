...
branch_reso = pi/256; % terminate bnb early when branch size < branch_reso
sample_reso = pi/256; % resolution for interval analysis
epsilon_r = 0.015;
q_list = [0.5,0.6,0.7,0.8,0.9];
L_list = q_list./(1-q_list)/epsilon_r;
num_q = length(q_list);
...
%%
% store results for likelihood-based saturation function in a temp buffer
...
parfor num = 1:total_img
    ...
    Record_CM_FGO(num,:)={img_idx,size(lines2D,1),epsilon_r,outlier_ratio,retrived_err_rot,max_err,min_err,num_candidate,best_score,gt_score,time,{rotmat2vec3d(R_max)}};
    ...
    [R_opt_top,best_score,num_candidate,time,~,~] = ...
        ...
        branch_reso,epsilon_r,sample_reso,prox_thres,west_east_flag);
    [min_err,max_err,R_min,R_max]=min_max_rot_error(num_candidate,R_opt_top,R_gt);
    Record_SCM_trunc(num,:)={img_idx,size(lines2D,1),epsilon_r,outlier_ratio,retrived_err_rot,max_err,min_err,num_candidate,best_score,gt_score,time,{rotmat2vec3d(R_max)}};
    ...
    for k = 1:num_q
        ...
        [min_err,max_err,R_min,R_max]=min_max_rot_error(num_candidate,R_opt_top,R_gt);
        temp_buffer{num,k}={img_idx,size(lines2D,1),epsilon_r,outlier_ratio,retrived_err_rot,max_err,min_err,num_candidate,best_score,gt_score,time,{rotmat2vec3d(R_max)}};
    end
end
%%
% organize data table
...
for k = 1:num_q
    fprintf("SCM_FGO(q=%f,L=%f): %d \n", q_list(k),L_list(k),length(find(Record_SCM_ML_lists{k}.("Max Rot Err")<10)))
end
...
for k = 1:num_q
    fprintf("SCM_FGO(q=%f,L=%f): %f,%f,%f\n", q_list(k),L_list(k),quantile(Record_SCM_ML_lists{k}.("Max Rot Err"),[0.25,0.5,0.75]))
end
...
for k = 1:num_q
    fprintf("SCM_FGO(q=%f,L=%f): %f,%f,%f\n", q_list(k),L_list(k),sum(Record_SCM_ML_lists{k}.("Max Rot Err")<[3,5,10])/num_valid_images*100)
end
