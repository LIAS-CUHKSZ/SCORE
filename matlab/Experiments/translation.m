...
total_img=2000;
column_names= ["Image Id","# 2D lines","epsilon_t","Outlier Ratio","Trans Err","BnB Score","GT Score","time"];
...
parfor num = 1:2000
    ...
    Record_gt_CM(num,:)={img_idx,M,epsilon_t,outlier_ratio,t_err,best_score,score_gt_CM,time};
    ...
    t_fine_tuned = tune_t(t_best_candidates,pert_rot_n_2D_inlier,endpoints_3D_inlier(1:2:end,:),epsilon_t);
    t_err = norm(t_fine_tuned-t_gt);
    Record_gt_SCM_trunc(num,:)={img_idx,M,epsilon_t,outlier_ratio,t_err,best_score,score_gt_SCM_trunc,time};
    ...
    for k = 1:num_q
        ...
        t_fine_tuned = tune_t(t_best_candidates,pert_rot_n_2D_inlier,endpoints_3D_inlier(1:2:end,:),epsilon_t);
        t_err = norm(t_fine_tuned-t_gt);
        temp_buffer{num,k}={img_idx,M,epsilon_t,outlier_ratio,t_err,best_score,score_gt_SCM_ML,time};
    end
end
%%
% organize data table
...
fprintf("Quantiles of outlier ratio:%f,%f,%f\n", quantile(Record_gt_CM.("Outlier Ratio"),[0.25,0.5,0.75]));
...
for k=1:num_q
    fprintf("SCM_gt_FGO_ML(q=%f,L=%f): %d \n", const*L_list(k)/(1+const*L_list(k)),L_list(k),length(find(Record_SCM_ML_lists{k}.("Trans Err")<0.1)))
end
%%
...
for k=1:num_q
    fprintf("SCM_gt_FGO_ML(q=%f,L=%f): %f,%f,%f\n",const*L_list(k)/(1+const*L_list(k)),L_list(k),quantile(Record_SCM_ML_lists{k}.("Trans Err"),[0.25,0.5,0.75]))
end
...
for k=1:num_q
    fprintf("SCM_gt_FGO_ML(q=%f,L=%f): %f,%f,%f\n", const*L_list(k)/(1+const*L_list(k)),L_list(k),sum(Record_SCM_ML_lists{k}.("Trans Err")<[0.05,0.1,0.2])/num_valid_images*100)
end
