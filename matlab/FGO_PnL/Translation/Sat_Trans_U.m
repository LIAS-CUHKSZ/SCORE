...
function [upper_bound] = Sat_Trans_U(pert_rot_n,p_3D,ids,epsilon_t,br_,space_size,kernel_buffer)
N = size(pert_rot_n,1);
    ...
    if isempty(ids_upper) % no valid interval
        ...
        [upper_bound,~] = saturated_interval_stabbing(intervals_upper,ids_upper,kernel_buffer,100);
    end
end
