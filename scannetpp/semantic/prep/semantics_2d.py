...
def main(cfg : DictConfig) -> None:
    ...
    obj_ids_dir = save_dir / 'obj_ids'

    for dir in [img_crop_dir, bbox_img_dir, viz_obj_ids_dir, obj_ids_dir]:
    ...
    for scene_id in tqdm(scene_list, desc='scene'):
        ...
        for _, image_name in enumerate(tqdm(image_list, desc='image', leave=False)):
            ...
            if cfg.dbg.obj_ids: # save obj ids to file
                out_path = obj_ids_dir / scene_id / f'{image_name}.npy'
                if not out_path.parent.exists():
                    out_path.parent.mkdir(parents=True, exist_ok=True)
                np.save(out_path, pix_obj_ids)
