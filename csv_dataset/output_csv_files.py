...
data_path = home+f"/SCORE/line_map_extractor/out/"+scene_id+"/results_merged.npy"
...
pose_folder = path+"poses/"
if not os.path.exists(pose_folder):
    os.makedirs(pose_folder)
intrinsic_folder = path+"intrinsics/"    
if not os.path.exists(intrinsic_folder):
    os.makedirs(intrinsic_folder)
line2d_folder = path+"lines2d/"
if not os.path.exists(line2d_folder):
    os.makedirs(line2d_folder)
#
while(len(scene_line_2d_semantic_labels)>0):
    ...
    lines_2d_thisframe = np.empty([num_lines,6],dtype=float)
    lines_2d_end_points = np.empty([2*num_lines,2],dtype=float)
...
lines_3d_all = np.empty([num_3d_line,7],dtype=float) # position, direction, semantics
...
df_3d.to_csv(path+'3dlines.csv', index=False)
...
df_3d_end.to_csv(path+'3dlines_endpoints.csv', index=False)
