...
Eigen::Matrix3d rodrigues(double alpha, double phi, double theta) {
    // Eigen::Matrix3d R;
    // Eigen::Vector3d axis(sin(alpha) * cos(phi), sin(alpha) * sin(phi), cos(alpha));
...
vector<double> interval_intersection(double &interval_1_left,double &interval_1_right, double &interval_2_left,double &interval_2_right){
    vector<double> intersection;
    intersection.reserve(2);
    if (interval_1_right < interval_2_left || interval_2_right < interval_1_left) {
        return intersection;
    }
