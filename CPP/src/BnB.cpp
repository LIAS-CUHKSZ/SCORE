...
void FGO_PnL::preprocess(line_pair& lp){
    ...
    if(outer_angle[1]>M_PI){
        ...
        lp.attribute->angle_east_outer_product=Vector2d(M_PI-outer_angle[0],outer_angle[1]-M_PI);
        ...
        lp.attribute->angle_west_outer_product=Vector2d(M_PI-outer_angle[0],outer_angle[1]+M_PI);
...
void FGO_PnL::rot_bnb_estimate(){
    ...
    while(!pq.empty()){
        ...
        // cout<<" ["<<branch_i.alpha.lower/M_PI<<", "<<branch_i.alpha.upper/M_PI<<", "<<branch_i.phi.lower/M_PI<<", "<<branch_i.phi.upper/M_PI<<" ]";
        // cout<<"UB: "<<branch_i.upper_bound<<" LB: "<<this->lower_bound<<endl;
        pq.pop();



        if(branch_i.upper_bound == lower_bound || branch_i.alpha.width<= this->branch_resolution ){
            // vec_rot = I_find_u;
            this->R_hat=rodrigues(this->vec_rot[0],this->vec_rot[1],this->vec_rot[2]);
            cout<<"output: alpha:"<<this->vec_rot[0]<<"  phi:"<<this->vec_rot[1]<<"  theta:"<<this->vec_rot[2]<<endl;
            cout<<this->R_hat<<endl;
            break;
        }else if(branch_i.upper_bound>lower_bound  ){
            Square branch1(range(branch_i.alpha.lower,branch_i.alpha.center),range(branch_i.phi.lower,branch_i.phi.center));
            Square branch2(range(branch_i.alpha.center,branch_i.alpha.upper),range(branch_i.phi.lower,branch_i.phi.center));
            Square branch3(range(branch_i.alpha.lower,branch_i.alpha.center),range(branch_i.phi.center,branch_i.phi.upper));
            Square branch4(range(branch_i.alpha.center,branch_i.alpha.upper),range(branch_i.phi.center,branch_i.phi.upper));
            rot_bnb_epoch(branch1);
            rot_bnb_epoch(branch2);
...
void FGO_PnL::rot_bnb_epoch(Square &branch){
    ...
    VectorXd u_center = polar_2_xyz(Vector2d(branch.alpha.center,branch.phi.center));
    for(auto i =0;i<N;i++){
        h1_center[i] = u_center.dot(init_line_pairs[i].attribute->outer_product);
        h2_center[i] = u_center.dot(init_line_pairs[i].line_3d)*u_center.dot(init_line_pairs[i].line_2d)-init_line_pairs[i].attribute->inner_product;
    }
    ...
    for (int i = 0; i < N; ++i) {
        ...
        upper_intervals.insert(upper_intervals.end(), tmp_interval.begin(), tmp_interval.end());
        line_tags_upper.insert(line_tags_upper.end(), tmp_interval.size() / 2, init_line_pairs[i].line_tag);

        // Lower intervals
        tmp_interval = lower_interval(center_params.row(i));
        lower_intervals.insert(lower_intervals.end(), tmp_interval.begin(), tmp_interval.end());
        line_tags_lower.insert(line_tags_lower.end(), tmp_interval.size() / 2, init_line_pairs[i].line_tag);
    }
    auto [Q_upper,_] = satured_interval_stabbing(upper_intervals,line_tags_upper);
    auto [Q_lower,theta_lower] = satured_interval_stabbing(lower_intervals,line_tags_lower);
...
pair<VectorXd,VectorXd> FGO_PnL::h1_bounds(Square &branch){
    ...
    if (cube_width <= sample_resolution) {
        for (int i = 0; i < N; ++i) {
            int flag = 1;
            if ((range_phi.upper > M_PI && !init_line_pairs[i].attribute->east_or_not) || (range_phi.upper <= M_PI && init_line_pairs[i].attribute->east_or_not)) {
                flag = 1;
            } else {
                flag = -1;
            }
            ...
            auto [alpha_far, alpha_near] = interval_projection(outer_angle[0], range_alpha);
            ...
            if (delta_phi_near ==0){
                ...
                maximum = max(outer_product.dot(polar_2_xyz(range_alpha.lower,phi_near)),outer_product.dot(polar_2_xyz(range_alpha.upper,phi_near)));
            }
            double delta_phi_far = abs(phi_far -outer_angle[1]);
            minimum = min(outer_product.dot(polar_2_xyz(range_alpha.lower,phi_far)),outer_product.dot(polar_2_xyz(range_alpha.upper,phi_far)));
            if (flag == 1) {
                upper(i) = maximum;
                lower(i) = minimum;
            } else {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            if (delta_phi_near == 0) {
                ...
                if (max_alpha < 0) max_alpha += M_PI;
                
                max_alpha = (max_alpha <= range_alpha.center) ? range_alpha.upper : range_alpha.lower;
                maximum = outer_product.dot(polar_2_xyz(max_alpha, phi_near));
            } else if(delta_phi_near < M_PI/2 && outer_angle[0]<M_PI/2 && range_alpha.lower>=outer_angle[0]){
                maximum = outer_product.dot(polar_2_xyz(range_alpha.lower, phi_near));
            } else if(delta_phi_near < M_PI/2 && outer_angle[0]>M_PI/2 && range_alpha.lower<=M_PI-outer_angle[0]){
                maximum = outer_product.dot(polar_2_xyz(range_alpha.upper, phi_near)); 
            }else if (delta_phi_near==M_PI/2){
                maximum = (outer_angle[0] <= M_PI/2) ? outer_product.dot(polar_2_xyz(range_alpha.lower, phi_near)): outer_product.dot(polar_2_xyz(range_alpha.upper, phi_near));
            }else{
                double tangent = std::tan(outer_angle[0]) * std::cos(delta_phi_near);
                double max_alpha = (tangent > 1e8) ? M_PI/2 : std::atan(tangent);
                if (max_alpha < 0) max_alpha += M_PI;
            ...
            if (delta_phi_far < M_PI/2) {
                ...
                if (min_alpha < 0) min_alpha += M_PI;
            
                if (min_alpha <= range_alpha.center) {  // sum(range_alpha)/2 用center代替
...
pair<VectorXd,VectorXd> FGO_PnL::h2_bounds(Square &branch){
    ...
    if (cube_width <= sample_resolution) {
        ...
        vector<double> alpha = generate_range(branch.alpha.lower, branch.alpha.upper, sample_resolution);
        vector<double> phi = generate_range(branch.phi.lower, branch.phi.upper, sample_resolution);
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (branch.phi.lower < M_PI) {
            ...
            o_normal_angle = init_line_pairs[i].attribute->angle_east_othogonal_angle_bisector;
            o_normal_vector = init_line_pairs[i].attribute->east_othogonal_angle_bisector;
        } else {
            ...
            o_normal_angle = init_line_pairs[i].attribute->angle_west_othogonal_angle_bisector;
            o_normal_vector = init_line_pairs[i].attribute->west_othogonal_angle_bisector;
        }
        ...
        RowVectorXd tmp = (n_i.transpose() * vertex_cache).array()*(v_i.transpose()*vertex_cache).array();
...
vector<double> FGO_PnL::upper_interval(RowVector3d coef_1,RowVector3d coef_2){
    ...
    for (size_t i = 0; i < upper_intervals.size(); i += 2) {
        if (i+1 >= upper_intervals.size()) break;
        for (size_t j = 0; j < lower_intervals.size(); j += 2) {
            if (j+1 >= lower_intervals.size()) break;
            vector<double> intersect = interval_intersection(upper_intervals[i], upper_intervals[i+1], lower_intervals[j], lower_intervals[j+1]);
...
pair<double,double> FGO_PnL::satured_interval_stabbing(vector<double> &interval,vector<int> line_tags){
    ...
    sort(sidx.begin(), sidx.end(), [&](double a, double b) {
...
void FGO_PnL::angle_bisectors(line_pair &lp){
    ...
    if (normal[1]> M_PI) {
        ...
        lp.attribute->angle_east_angle_bisector = Vector2d(M_PI-normal[0],normal[1]-M_PI);
        lp.attribute->east_angle_bisector = polar_2_xyz(lp.attribute->angle_east_angle_bisector);
        lp.attribute->west_angle_bisector = polar_2_xyz(lp.attribute->angle_west_angle_bisector);
    } else {
        lp.attribute->angle_west_angle_bisector = Vector2d(M_PI-normal[0],normal[1]+M_PI);
    ...
    if(o_normal[1]>M_PI){
        ...
        lp.attribute->angle_east_othogonal_angle_bisector = Vector2d(M_PI-o_normal[0],o_normal[1]-M_PI);
        lp.attribute->east_othogonal_angle_bisector = polar_2_xyz(lp.attribute->angle_east_othogonal_angle_bisector);
        lp.attribute->west_othogonal_angle_bisector = polar_2_xyz(lp.attribute->angle_west_othogonal_angle_bisector);
    }else{
        lp.attribute->angle_west_othogonal_angle_bisector = Vector2d(M_PI-o_normal[0],o_normal[1]+M_PI);
...
MatrixXd FGO_PnL::generate_params(VectorXd &lower,VectorXd &upper){
    ...
    VectorXd phi = (-h2).binaryExpr(h1, [](double a, double b) { return atan2(a, b); });
