...
int main() {
  ...
  FGO_PnL *solver = new FGO_PnL(max_iter, rotation_epsilon, branch_resolution,
                                sample_resolution, translation_epsilon,
                                translation_resolution, satured_kernel);
  ...
  Vector3d line2d;
  Vector3d line3d;
  // combined_data = [id_cluster, data_2d_N_clustered, data_3d_v_clustered]; %
  // 合并为 N×7 矩阵
  while (getline(file, line)) {
    stringstream ss(line);

    ss >> line_tag >> comma >> line2d[0] >> comma >> line2d[1] >> comma >>
        line2d[2] >> comma >> line3d[0] >> comma >> line3d[1] >> comma >>
        line3d[2];
